...
public sealed class UsageCoordinatorTests
{
    ...
    [Fact]
    public async Task Refresh_does_not_invoke_disabled_providers()
    {
        var now = new DateTimeOffset(2026, 7, 16, 12, 0, 0, TimeSpan.Zero);
        var history = new RecordingHistoryStore();
        var disabledCalls = 0;
        ...
        {
            new StubProvider("claude", "Claude", () => Snapshot("claude", "Claude", now)),
            new StubProvider("codex", "Codex", () =>
            {
                disabledCalls++;
                return Snapshot("codex", "Codex", now);
            })
        };
        var coordinator = new UsageCoordinator(providers, history, new FixedTimeProvider(now));

        var results = await coordinator.RefreshAsync(["claude"], CancellationToken.None);

        Assert.Single(results);
        Assert.Equal("claude", results[0].ProviderId);
